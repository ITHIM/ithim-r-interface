...
packages <- c("tidyverse", "earth", "ithimr", "parallel")
...
for (scenario in scenarios) {
  ithim_object <- run_ithim_setup(seed = parameters$seed,
...
ithim_object$outcomes <- lapply(1:NSAMPLES, run_ithim, ithim_object = ithim_object)
ithim_object$outcomes <- lapply(ithim_object$outcomes, function(x) x$hb)
ithim_object$parameters <- NULL
assign(paste0("ithim_object_", scenario), ithim_object)
}

#Call the main analysis results
ithim_object_now <- readRDS("./now.Rds")
ithim_object_now$outcomes <- lapply(ithim_object_now$outcomes, function(x) x$hb)
ithim_object_now$parameters <- NULL
